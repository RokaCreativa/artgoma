...
const FilterByName = ({ visits, setFilteredData }: { visits: IVisit[]; setFilteredData: React.Dispatch<IVisit[]> }) => {
    ...
    setTypingTimeout(
      setTimeout(() => {
        ...
        if (term.trim() === "") {
          ...
          setFilteredData(visits.filter((item: IVisit) => item.User?.name.toLowerCase().includes(term.toLowerCase())));
        }
      }, 1500)
    );
  };
